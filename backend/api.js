      ...
      if (result.modifiedCount === 1) {
        if (decision === "REJECTED"){
          ...
          const result3 = await coll2.updateOne({
            "logInfo.username": result2.username
          }, {
            $inc: {
              "workInfo.holidayBalance": result2.days
            }
          });
    ...
    try {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        const result2 = await coll2.findOne({
          "logInfo.username": current.username
        });
  ...
  app.post("/api/notifications/dismiss", async (req, res) => {
    ...
    try {
      ...
      const user = await collection.findOne({
        "logInfo.username": Registry.getInstance().getUsers().get(token).getUName()
      });
      ...
      if (user.notifications.includes(notification)) {
        ...
        const result = await collection.updateOne({
          "logInfo.username": Registry.getInstance().getUser(token).getUName()
        }, {
          $set: {
...
const getRemainingLeaveEndpoint = (app) => {
  ...
  app.get("/api/leave/remaining", async (req, res) => {
    ...
    try {
      ...
      res.send({
        status: "success",
        remaining: user.workInfo.holidayBalance
      });
    } catch (err) {
      console.log(err);
    } finally {
...
const editProfileEndpoint = (app) => {
  app.post("/api/profile/save", async (req, res) => {
    ...
    try {
      ...
      const result = await collection.updateOne({
        "logInfo.username": user.getUName()
      }, {
        $set: {
...
const loginEndpoint = (app) => {
  app.post("/api/login", async (req, res) => {
    ...
    try {
      await client.connect();
      const db = client.db("e-portal");
      const collection = db.collection("users");
      const user = await collection.findOne({ "logInfo.username": username });
      const hash = crypto.createHash('sha256').update(password).digest('base64');
